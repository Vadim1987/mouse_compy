...
function pop_radius(row)
  local d = math.max(row.size * APP.width, POP.min_px)
  return d / 2
end

function pop_sample(r)
  local x = rand_range(r, APP.width - r)
  local y = rand_range(r, APP.height - r)
  return x, y
end
...
function pop_far_point(r, row)
  ...
  while true do
    ...
    local dx, dy = x - bubble.x, y - bubble.y
...
function pop_add_particle(i)
  local a = rand_range(0, 2 * math.pi)
  local sp = rand_range(BURST.speed * 0.5, BURST.speed)
  bubble.burst[i] = {
    x = bubble.x,
    y = bubble.y,
    vx = math.cos(a) * sp,
...
function pop_hit()
  ...
  pop_spawn_burst()
  local row = pop_row()
  bubble.new_x, bubble.new_y = pop_far_point(pop_radius(row), row)
...
function pop_miss()
  ...
  if not bubble.struggled
     and POP.struggle_clicks <= bubble.off then
...
function pop_live(dt)
  ...
  pop_drift(dt)
  if not bubble.struggled
     and POP.struggle_t <= bubble.age then
    bubble.struggled = true
    notch_report("pop", "struggle")
  end
end
...
function pop_anim(dt)
  ...
  if pop_row().respawn <= bubble.t then
    pop_begin(bubble.new_x, bubble.new_y,
      pop_radius(pop_row()))
  end
end
...
function pop_draw_bubble()
  ...
  gfx.circle("fill", bubble.x - r * POP.hi_off,
    bubble.y - r * POP.hi_off, r * POP.hi_r)
end
